...
def cargar_datos():
    ...
    df["comision_airbnb"] = pd.to_numeric(df.get("tarifa_servicio", 0), errors="coerce").fillna(0)
    df["ingreso_bruto"]   = pd.to_numeric(df["ingreso_bruto"],  errors="coerce").fillna(0)
    df["ingreso_real"]    = pd.to_numeric(df["ingreso_real"],   errors="coerce").fillna(0)
...
meses_ordenados = sorted(df["mes_nombre"].unique().tolist(), key=lambda x: orden_meses.get(x, 99))
...
st.markdown(f"📅 Dic 2025 — Mar 2026 | **{df_f['propiedad'].nunique()}** propiedades | **{len(meses_sel)}** meses seleccionados")
st.markdown("---")

# ── MÉTRICAS PRINCIPALES ─────────────────────────────────
total_bruto    = df_f["ingreso_bruto"].sum()
total_real     = df_f["ingreso_real"].sum()
...
c2.metric("📈 Ingreso neto real",   f"${total_real:,.0f}")
c3.metric("✂️ Comisión Airbnb",    f"${total_comision:,.0f}",
          delta=f"-{pct_comision:.1f}% del bruto", delta_color="inverse")
c4.metric("🛏️ Noches vendidas",    f"{total_noches:,.0f}")
c5.metric("📋 Reservaciones",       f"{total_reserv:,}")

st.markdown("---")

# ── SECCIÓN DINÁMICA SEGÚN VISTA ─────────────────────────
...
if vista == "Todas las propiedades":
    ...
    with col1:
        st.subheader("💰 Ingreso neto real por propiedad")
        res = df_f.groupby("propiedad")["ingreso_real"].sum().sort_values()
        promedio = res.mean()
    ...
    with col2:
        ...
        fig, ax = plt.subplots(figsize=(7, 5))
        bars = ax.barh(com.index, com["comision"], color="#e74c3c")
    ...
    with col3:
        st.subheader("📈 Tendencia ingresos por mes")
        tend = df_f.groupby(["mes_num","mes_nombre","propiedad"])["ingreso_real"].sum().reset_index()
        fig, ax = plt.subplots(figsize=(7, 5))
        for prop in propiedades_sel:
            d = tend[tend["propiedad"]==prop].sort_values("mes_num")
            if len(d) > 0:
                ax.plot(d["mes_nombre"], d["ingreso_real"],
    ...
    with col1:
        ...
        ax.bar([i + w/2 for i in x], mes_prop["real"],   width=w,
    ...
    with col2:
        ...
        bars = ax.bar(mes_prop["mes_nombre"], mes_prop["comision"],
                     color="#e74c3c")
        for bar, pct in zip(bars, (mes_prop["comision"]/mes_prop["bruto"]*100).round(1)):
            ax.text(bar.get_x() + bar.get_width()/2,
                   bar.get_height() + 200,
                   f"${bar.get_height():,.0f}\n({pct}%)",
                   ha="center", fontsize=8)
    ...
    st.subheader("📋 Reservaciones de esta propiedad")
    tabla_prop = df_prop[["fecha_inicio","mes_nombre","noches",
                          "ingreso_bruto","comision_airbnb","ingreso_real"]].copy()
    tabla_prop.columns = ["Fecha","Mes","Noches","Bruto","Comisión","Neto"]
    tabla_prop = tabla_prop.sort_values("Fecha", ascending=False)
    ...
    m_real     = df_mes["ingreso_real"].sum()
    m_comision = df_mes["comision_airbnb"].sum()
    m_noches   = df_mes["noches"].sum()
    m1.metric("💰 Ingreso bruto",    f"${m_bruto:,.0f}")
    m2.metric("📈 Ingreso neto",     f"${m_real:,.0f}")
    m3.metric("✂️ Comisión Airbnb", f"${m_comision:,.0f}",
              delta=f"-{(m_comision/m_bruto*100):.1f}%", delta_color="inverse")
    m4.metric("🛏️ Noches vendidas", f"{m_noches:,.0f}")

    col1, col2 = st.columns(2)

    with col1:
        st.subheader("💰 Ingreso neto por propiedad")
        res_mes = df_mes.groupby("propiedad")["ingreso_real"].sum().sort_values()
    ...
    tabla_mes = df_mes.groupby("propiedad").agg(
        ...
        Bruto         = ("ingreso_bruto",   "sum"),
        Comision      = ("comision_airbnb", "sum"),
        Neto          = ("ingreso_real",    "sum")
    ).round(0).sort_values("Neto", ascending=False)
...
col_t2.metric("📈 Total neto",         f"${total_real:,.0f} MXN")
col_t3.metric("✂️ Total comisión",    f"${total_comision:,.0f} MXN")
col_t4.metric("📊 % Comisión prom",   f"{pct_comision:.1f}%")
